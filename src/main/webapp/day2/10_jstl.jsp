...
<body>
	...
	<p>
	기존에 사용하던 jsp의 스크립트릿,출력식 기호를 대체합니다.
	이유는 요청처리하는 자바코드와 화면을 구성하는 html 태그를 분리할 때
	jsp 파일을 태그형식으로만 작성하기 위함입니다.
	</p>
	<p>태그 라이브러리는 맨 위 지시자에서 정의하고 사용합니다. EL 기호 애트리뷰트 값만 출력할 수 있습니다.</p>
	...
	<h5> name : <c:out value="${name }"></c:out> </h5>	<!-- name은 자바변수 출력안됨. -->
	...
	<!-- 5.for문 -->
		...
		<div>
			...
			<!-- 객체변수명.getXXX() getter 메소드 사용합니다. 그러나 EL은
				알아서 getter 동작을 합니다. 객체의 속성이름을 정확히 작성하기
			 -->
				<li>${customer.customId }</li>
				<li>${customer.name }</li>
				<li>${customer.email }</li>
				<li>${customer.age }</li>
				<li>${customer.reg_date }</li>
			</ul>
